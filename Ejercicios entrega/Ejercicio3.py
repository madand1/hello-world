discos_metallica = [{
    ...
    # Número de discos de Metallica
num_discos = len(discos_metallica)
print(f"Número de discos de Metallica: {num_discos}")

# Información detallada de cada disco
for disco in discos_metallica:
    nombre_disco = disco["album"]["name"]
    num_canciones = len(disco["album"]["tracks"]["track"])
    reproducciones = disco["album"]["playcount"]
    
    print(f"\nDisco: {nombre_disco}")
    print(f"Número de canciones: {num_canciones}")
    print(f"Número de reproducciones: {reproducciones}")

# Pedir por teclado el nombre de un disco y mostrar títulos de canciones
nombre_disco_input = input("\nIntroduce el nombre de un disco de Metallica: ")
encontrado = False

for disco in discos_metallica:
    if disco["album"]["name"] == nombre_disco_input:
        encontrado = True
        print(f"\nCanciones del disco '{nombre_disco_input}':")
        for cancion in disco["album"]["tracks"]["track"]:
            print(f"- {cancion['name']}")
        break

if not encontrado:
    print(f"\nError: No se encontró el disco '{nombre_disco_input}'.")

# Pedir por teclado una etiqueta y mostrar títulos de discos y URLs de imágenes
etiqueta_input = input("\nIntroduce una etiqueta: ")
discos_con_etiqueta = []

for disco in discos_metallica:
    if "tags" in disco["album"] and "tag" in disco["album"]["tags"]:
        etiquetas = [tag["name"] for tag in disco["album"]["tags"]["tag"]]
        if etiqueta_input in etiquetas:
            discos_con_etiqueta.append(disco)

if discos_con_etiqueta:
    print(f"\nDiscos con la etiqueta '{etiqueta_input}':")
    for disco in discos_con_etiqueta:
        nombre_disco = disco["album"]["name"]
        urls_imagenes = [imagen["#text"] for imagen in disco["album"]["image"]]
        print(f"\nDisco: {nombre_disco}")
        print(f"URLs de imágenes: {', '.join(urls_imagenes)}")
else:
    print(f"\nNo se encontraron discos con la etiqueta '{etiqueta_input}'.")